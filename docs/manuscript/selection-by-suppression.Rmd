...
Despite the long-term effects of fire suppression on wildfire trends, fire suppression generally is very effective at its immediate goal of extinguishing fires. Between 1970 and 2002, over 97% of fires burning on U.S. Forest Service land were contained during before they reached 120 hectares in size [@calkin2005; @calkin2015]. 

In the western U.S., including the Sierra yellow pine/mixed-conifer system, the vast majority of fires are managed for suppression [@calkin2015]. The "10 a.m. policy", which dictated that fires should be put out before 10 a.m. of the day following their discovery, was established in the 1930's and has since been modified, but its aggressive spirit in managing fire persists in today's modern firefighting apparatus [@dale2006; @johnson2009].

Wildfire effects to forest vegetation are outcomes of a complex social-ecological system dynamic [@calkin2015]. Direct causes of fire effects to vegetation arise from fire behavior and intensity [@keeley2009], which are coupled to fuel, weather, and topography [@mckenzie2008]. Wildfire effects are indirectly related to resource availability, legacies of management policy, and incentive structures that prioritized short term loss of timber resources over long-term benefits of wildfire [@houtman2013; @calkin2014; @calkin2015].
...
Many studies have suggested that adding more fire to the landscape is a way to return forests to pre-Euroamerican settlement resilient conditions [@mallek2013; @meyer2015; @north2015; @stevens2017; @collins2017] and some efforts are underway though barriers remain [@doane2006; @calkin2015].

We use a new dataset of fire severity [@koontz2019] to measure this "selection by suppression" effect and ask:

1) What is the strength of "selection" on wildfire burning conditions (regional climate, vegetation density, vegetation continuity, concurrently burning fires) imposed by initial attack suppression efforts?
...
A small number of natural ignitions are allowed to burn under moderate conditions in "resource benefit", "resource objective", or "wildland fire use" fires in recognition of the important role that fire plays in many healthy forest systems [@davis1979; @meyer2015].
...
### Context of suppression fires compared to wildfire use fires

There is a clear multimodality in the fire event size between fires that are managed for suppression versus fires that are managed for resource benefit (i.e., wildfire use fires) (`r fig_nums(name = "fig-fires-size-histogram-by-management-objective", display = "cite")`.

```{r}
fig_nums(name = "fig-fires-size-histogram-by-management-objective", caption = "Distribution of log fire event size by management objective. While wildfire use fires exhibit a lognormal distribution in size, suppression fires exhibit clear multimodality with many fires extinguished when they are very small.")
```

![`r fig_nums(name = "fig-fires-size-histogram-by-management-objective")`](../../figures/fire-size-histogram-by-management-objective.png)
...
```{r}
fig_nums(name = "selection-by-suppression-phenotype-distributions", caption = "The selection effect on the vegetation and climate burning conditions of suppressed wildfires.")
```
